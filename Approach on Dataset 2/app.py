...
# Load models
model_paths = ["models/alexnet.h5", "models/resnet50.h5", "models/unet.h5"]
models = [load_model(path) for path in model_paths]

# Updated class labels for brain tumor types
class_labels = ['Glioma', 'Meningioma', 'Tumor']

# Function to preprocess image
def preprocess_image(image):
    # Resize image to (128, 128) and convert to RGB if needed
...
def get_unet_class_prediction(mask):
    ...
    # Example: Assuming segmentation classes are labeled as 0, 1, 2
    glioma_count = np.sum(flat_mask == 0)
    meningioma_count = np.sum(flat_mask == 1)
    tumor_count = np.sum(flat_mask == 2)

    # Get the predicted class based on maximum count
    counts = [glioma_count, meningioma_count, tumor_count]
...
st.write("Upload an MRI image to predict the type of brain tumor (Glioma, Meningioma, Tumor).")
